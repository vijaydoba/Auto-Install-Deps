import * as vscode from "vscode";

const phpUseRegex =
    /^\s*use\s+(?:function\s+|const\s+)?([A-Za-z0-9_\\]+)\s*;/gm;

export function collectPhpDiagnostics(
    document: vscode.TextDocument,
    workspacePath: string,
): vscode.Diagnostic[] {
    const diagnostics: vscode.Diagnostic[] = [];
    const text = document.getText();

    const usedPackages = new Map<string, vscode.Range[]>();
    let match: RegExpExecArray | null;

    while ((match = phpUseRegex.exec(text)) !== null) {
        const full = match[1]; // e.g. Vendor\Package\Class
        const parts = full.split("\\");
        if (parts.length === 0) continue;

        const vendor = parts[0].toLowerCase();
        const pkg = (parts[1] || parts[0]).toLowerCase();
        const composerName = `${vendor}/${pkg}`;

        addOccurrence(
            document,
            usedPackages,
            composerName,
            match.index,
            match[0].length,
        );
    }

    for (const [pkgName, ranges] of usedPackages.entries()) {
        for (const range of ranges) {
            const diag = new vscode.Diagnostic(
                range,
                `PHP dependency "${pkgName}" may not be installed (Composer).`,
                vscode.DiagnosticSeverity.Warning,
            );
            (diag as any).code = {
                value: "missingDeps",
                moduleName: pkgName,
                workspacePath,
                language: "php",
            };
            diagnostics.push(diag);
        }
    }

    return diagnostics;
}

function addOccurrence(
    document: vscode.TextDocument,
    map: Map<string, vscode.Range[]>,
    name: string,
    index: number,
    length: number,
) {
    const start = document.positionAt(index);
    const end = document.positionAt(index + length);
    const range = new vscode.Range(start, end);
    const existing = map.get(name) || [];
    existing.push(range);
    map.set(name, existing);
}
